...
- 台账树节点支持右键新建子节点、升级、降级、重命名、复制和安全删除；内部拖拽使用 Pointer Events，节点上沿、中央、下沿分别对应同级前插、设为子节点和同级后插，调整后的顺序会持久保留
- 文件列表支持拖动单个或多选文件到左侧任意台账节点，显示跟随鼠标的移动浮层与目标高亮，松开后快速移动
- 文件列表支持按修改时间、名称、文件类型、大小和有效期排序，支持升降序并记忆上次选择；无有效期资料在有效期排序中始终置后
- 自定义彩色标签、20 种预设标签色、任意颜色选择、文件名后标签胶囊、标签筛选和批量标签编辑
...
- 主页有效期中心集中展示已过期和 30 天内到期资料；右上角通知按钮打开悬浮通知中心，保留告警历史、未读状态和全部已读操作，不会重复触发同一通知
...
已经安装过应用时，无需卸载。关闭正在运行的“资料台账”，双击 `update-installed.cmd`，脚本会执行 Release 增量构建并静默原位升级。应用数据库和资料库不会被安装程序覆盖。
...
资料库默认保存在 Windows 应用数据目录，也可在设置中迁移到其他位置。应用打开的是内部保存的正式副本；在 Word 等程序中保存后，后台文件监视器会更新检索正文，因此标签和树状位置不会失效。
